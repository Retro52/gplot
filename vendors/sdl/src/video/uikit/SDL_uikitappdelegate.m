...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    /* Firebase subsribe to notifications */
...
- (void)messaging:(FIRMessaging *)messaging didReceiveRegistrationToken:(NSString *)fcmToken {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:fcmToken forKey:@"firebase_token"];
}
#endif
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    NSString *const forwardToUrlKey = @"forward_to_url";

    NSLog(@"userNotificationCenter.didReceiveNotificationResponse.withCompletionHandler");
    NSLog(@"%@", userInfo);

    // handle open external url if notification data payload contains corresponding tag
    if (userInfo[forwardToUrlKey]) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString: userInfo[forwardToUrlKey]]];
    }
    completionHandler();
}
#endif
