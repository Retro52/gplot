...
namespace gplot::graphics
{
    ...
    public:
        ...
        Mesh(const std::array<Vertex, Vertices>& vertices, const std::array<GLuint, Indices>& indices)
            : Mesh(vertices.data(), vertices.size(), indices.data(), indices.size())
        {
        }
    ...
    private:
        ...
        Mesh(const Vertex* vert_data, size_t vert_size, const GLuint* ind_data, size_t ind_size)
            : m_count(ind_size)
